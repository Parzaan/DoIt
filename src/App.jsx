    ...
    <>
      ...
      <div className="min-h-screen flex flex-col items-center py-20 px-4">
          ...
          {isCatModalOpen && (
            <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4">
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-sm glass rounded-3xl p-8">
                <h3 className="text-xl font-black mb-4 text-white uppercase tracking-tighter">Create Category</h3>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <h1 className="text-6xl font-black tracking-tighter bg-linear-to-b from-white to-slate-500 bg-clip-text text-transparent font-mono">DoIt.</h1>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-xl glass rounded-3xl p-8 space-y-6">
            ...
            <div className="flex gap-2">
              <AnimatePresence mode="wait">
                {tasks.some(t => t.completed) && (
                  !showClearConfirm ? (
                    <motion.button key="eraser" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={() => setShowClearConfirm(true)} className="p-2 bg-red-500/10 text-red-400 rounded-xl border border-red-500/20 hover:bg-red-500/20 transition-all"><Eraser size={18} /></motion.button>
                  ) : (
                    <motion.div key="confirm" initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-xl p-1 overflow-hidden">
                      <button onClick={clearCompleted} className="p-1.5 text-emerald-400 hover:bg-emerald-400/10 rounded-lg transition-colors"><Check size={16}/></button>
                      <button onClick={() => setShowClearConfirm(false)} className="p-1.5 text-slate-500 hover:bg-white/5 rounded-lg transition-colors"><RotateCcw size={16}/></button>
                    </motion.div>
                  )
                )}
              </AnimatePresence>
              <button onClick={() => setShowSearch(!showSearch)} className={`p-2 rounded-xl transition-all ${showSearch ? 'bg-cyan-500 text-black' : 'bg-white/5 text-slate-400'}`}><Search size={18} /></button>
            </div>
          </div>
                  ...
                  {["All", ...customCategories].map(cat => (
                    <div key={cat} className="relative group/cat shrink-0 flex items-center">
                      ...
                      {!["Personal", "Work", "Urgent"].includes(cat) && (
                        <button onClick={(e) => { e.stopPropagation(); deleteCategory(cat); }} className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/3 opacity-0 group-hover/cat:opacity-100 bg-red-500 text-white rounded-full p-1 shadow-2xl hover:bg-red-400 transition-all z-30 flex items-center justify-center border-2 border-black"><X size={10} strokeWidth={4} /></button>
                      )}
                    </div>
                  ))}
                </div>
          ...
          <div className="relative min-h-[200px]">
            {isTasksLoading ? (
              <div className="absolute inset-0 flex items-center justify-center"><motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="w-10 h-10 border-4 border-cyan-500/20 border-t-cyan-500 rounded-full" /></div>
            ) : (
              <Reorder.Group axis="y" values={tasks} onReorder={handleReorder} className="space-y-3">
                <AnimatePresence mode="popLayout">
                  {filteredTasks.map((task) => (
                    <Reorder.Item key={task.id} value={task} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="task-item group flex items-center justify-between bg-white/5 p-4 rounded-2xl border border-white/5 hover:border-white/20 transition-all active:cursor-grabbing">
                      <div className="flex items-center gap-4">
                        <GripVertical size={16} className="text-slate-700 cursor-grab group-hover:text-slate-400" />
                        <button onClick={() => toggleTask(task.id)} className={`${task.completed ? 'text-emerald-400' : 'text-slate-600'} relative`}>
                          <motion.div whileHover={{ scale: 1.3, rotate: 10 }} whileTap={{ scale: 0.8, rotate: -20 }} transition={{ type: "spring", stiffness: 400, damping: 10 }}><CheckCircle size={22} /></motion.div>
          ...
          <div className="pt-6 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4">
            {user ? (
              ...
              <div className="flex items-center justify-between w-full">
                ...
                <button onClick={() => setIsAuthOpen(true)} className="flex items-center gap-2 bg-cyan-600/20 text-cyan-300 px-6 py-2 rounded-full border border-cyan-500/30 hover:bg-cyan-600/40 text-sm font-medium transition-all uppercase tracking-widest">Sync Account <ArrowRight size={16} /></button>
              </div>
            )}
          </div>
        </motion.div>

        <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }} onClick={() => !user ? setIsAuthOpen(true) : exportToPDF()} className="mt-8 flex items-center gap-2 text-slate-600 hover:text-cyan-400 transition-colors text-sm uppercase font-bold tracking-widest">
